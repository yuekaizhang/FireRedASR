...
from fireredasr.models.fireredasr_llm_vllm import FireRedAsrLlm as FireRedAsrLlmVllm
...
class FireRedAsr:
    ...
    def from_pretrained(cls, asr_type, model_dir):
        assert asr_type in ["aed", "llm", "vllm"]
        ...
        if asr_type == "aed":
            ...
            encoder_path = os.path.join(model_dir, "asr_encoder.pth.tar")
            llm_dir = os.path.join(model_dir, "Qwen2-7B-Instruct")
            model, tokenizer = load_firered_vllm_model_and_tokenizer(
                model_path, encoder_path, llm_dir)
            # model.eval() # vLLM doesn't need explicit eval for generation usually, but encoder does.
            # FireRedAsrLlmVllm is nn.Module, so eval() affects encoder/projector/embedding.
            model.eval()
...
def load_firered_llm_model_and_tokenizer(model_path, encoder_path, llm_dir):
    ...
    args = package["args"]
    args.use_flash_attn = 1
    args.use_fp16 = 1
    model = FireRedAsrLlm.from_args(args)
    model.load_state_dict(package["model_state_dict"], strict=False)
    tokenizer = LlmTokenizerWrapper.build_llm_tokenizer(llm_dir, use_flash_attn=args.use_flash_attn)
    print("model args:", args)
    # new_llm_dir = os.path.join(os.path.dirname(llm_dir), "new_llm")
    # tokenizer.save_pretrained(new_llm_dir)
    # llm_merged = model.llm.merge_and_unload()
    # llm_merged.save_pretrained(new_llm_dir)

    return model, tokenizer


def load_firered_vllm_model_and_tokenizer(model_path, encoder_path, llm_dir):
    package = torch.load(model_path, map_location=lambda storage, loc: storage, weights_only=False)
    package["args"].encoder_path = encoder_path
    package["args"].llm_dir = llm_dir
    ...
    args.use_flash_attn = 1
    args.use_fp16 = 1
    model = FireRedAsrLlmVllm.from_args(args)
    # The strict=False is crucial as vllm LLM doesn't have state_dict
    model.load_state_dict(package["model_state_dict"], strict=False)
    tokenizer = LlmTokenizerWrapper.build_llm_tokenizer(llm_dir, use_flash_attn=args.use_flash_attn)
    print("model args:", args)
