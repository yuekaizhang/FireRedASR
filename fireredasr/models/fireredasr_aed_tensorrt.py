...
class TrtEncoder:
    def __init__(self, engine_path, dtype='float16'):
...
class TrtLlmDecoder:
    ...
    def generate(self,
        ...
        if self.decoder_config['plugin_config']['remove_input_padding']:
            print("remove_input_padding is True", 2333333)
            breakpoint()
...
class FireRedAsrAedTensorRT(torch.nn.Module):
    @classmethod
    def from_model_dir(cls, tensorrt_model_dir, tokenizer):
        return cls(tensorrt_model_dir, tokenizer)

    def __init__(self, tensorrt_model_dir, tokenizer):
        super().__init__()
        
        encoder_engine_path = os.path.join(tensorrt_model_dir, "encoder.plan")
        self.encoder = TrtEncoder(encoder_engine_path)

        decoder_engine_dir = Path(os.path.join(tensorrt_model_dir, "trt_engine_float16"))
