...
class FireRedAsrLlm(nn.Module):
    ...
    def from_args(cls, args):
        ...
        # Hardcode model path as requested
        model_name = "/workspace_yuekai/asr/FireRedASR/pretrained_models/FireRedASR-LLM-L/new_llm"
        print(f"Loading vLLM model from {model_name}", "*" * 100)
        llm = LLM(model=model_name, enable_prompt_embeds=True)
        print(f"vLLM model loaded", "*" * 100)
        ...
        # Load HF model to get embeddings layer and config
        # We assume we can load it alongside vLLM (watch out for VRAM)
        ...
        llm_config = hf_model.config
        llm_dim = llm_config.hidden_size

        # Initialize vLLM

        tokenizer = AutoTokenizer.from_pretrained(model_name)
        # tokenizer = LlmTokenizerWrapper.build_llm_tokenizer(model_name)
        assert tokenizer.pad_token_id == tokenizer.convert_tokens_to_ids("<|endoftext|>")
        llm_config.pad_token_id = tokenizer.pad_token_id
        llm_config.bos_token_id = tokenizer.convert_tokens_to_ids("<|im_start|>")
        llm_config.eos_token_id = tokenizer.convert_tokens_to_ids("<|im_end|>")
        llm_config.default_speech_token_id = tokenizer.convert_tokens_to_ids(
        ...
        return cls(encoder, llm, encoder_projector, embedding_layer, llm_config,
                   args.freeze_encoder, args.freeze_llm)

    def __init__(self, encoder, llm, encoder_projector, embedding_layer, llm_config,
    ...
    def transcribe(self, padded_feat, feat_lengths, padded_input_ids, attention_mask,
        ...
        inputs_embeds = self.embedding_layer(padded_input_ids)
    ...
    def _merge_input_ids_with_speech_features(
        ...
        left_padding = not torch.sum(
            input_ids[:, -1] == torch.tensor(self.llm_config.pad_token_id)
        )
        # 1. Create a mask to know where special speech tokens are
        special_speech_token_mask = input_ids == self.llm_config.default_speech_token_id
        ...
        batch_indices, non_speech_indices = torch.where(
            input_ids != self.llm_config.default_speech_token_id
        )
        ...
        batch_indices, pad_indices = torch.where(
            input_ids == self.llm_config.pad_token_id
        )
